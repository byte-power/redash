...
export const sessionRefreshInterceptor = createAuthRefreshInterceptor(
  ...
  error => {
    ...
    let needToken = status === 401 || includes(message, "Please login");
    if (includes(requestUrl, "access_token=") && needToken) {
      return Promise.reject(error);
    }
    if (error.isAxiosError && needToken && requestUrl !== "api/session") {
